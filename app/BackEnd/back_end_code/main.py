...
auth_str = base64.b64encode(credentials.encode("utf-8")).decode("utf-8")
...
def answer_question(question: str, session_id: str):
    ...
    tracking_response = handle_tracking(msg, engine_track, llm, session_id)
    ...
    for item in recent_history:
        role_name = "Gebruiker" if item["role"] == "user" else "Assistent"
        formatted_context += f"{role_name}: {item['message']}\n"

    full_input = f"Context van het gesprek:\n{formatted_context}\nNieuwe vraag: {msg}"
    ...
    if not response or not response.content:
        ...
        if result == "LATEST_ORDER":
            ...
            try:
                ...
                if field == "Address":
                    query = "SELECT street, HouseNumber, ZIPCODE, Country FROM afas.Bring_Employees WHERE FirstName = :name"
                else:
                    query = f"SELECT {field} FROM afas.Bring_Employees WHERE FirstName = :name"
                
                with engine_emp.connect() as conn:
                    row = conn.execute(text(query), {"name": name}).fetchone()
                
                if row:
                    answer = f"{row[0]} {row[1]}, {row[2]}, {row[3]}" if field == "Address" else str(row[0])
                else:
                    answer = f"Ik kon geen gegevens vinden voor {name}."
            except:
                answer = "Er trad een fout op bij het zoeken naar de medewerker."
        else:
            answer = result

    append_history(session_id, "user", msg)
    append_history(session_id, "assistant", str(answer))
...
async def chat(req: ChatReq):
    ...
    ans = await run_in_threadpool(lambda: answer_question(req.message, session_id))
